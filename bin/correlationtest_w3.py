...
The results of the test are labeled as significant basing on configurable thresholds
for pvalue and correlation absolute value.
'''
import pandas as pd
import logging
import argparse
...
def save_results(outfile, pvalue, coeff):
    ...
    with open(outfile, 'w') as output:
        output.write(f'\n SPEARMAN CORRELATION HP TEST- new vaccinations and deaths_vs_cases')
        output.write(f'\n pvalue: {pvalue}')
...
def main(processedcsvfile_w3: str, outfile: str, corrthr: float, pvalthr: float):
